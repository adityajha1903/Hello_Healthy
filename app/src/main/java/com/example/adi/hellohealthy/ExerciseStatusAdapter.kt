    ...
    override fun onBindViewHolder(holder: ViewHolderExerciseStatus, position: Int) {
        ...
        when {
            exercise.getIsCompleted() -> {
                holder.tvExerciseNumTv.background = ContextCompat.getDrawable(holder.itemView.context, R.drawable.ic_each_rv_ex_completed_green_bg)
                holder.tvExerciseNumTv.setTextColor(Color.WHITE)
            }
            exercise.getIsSelected() -> {
                holder.tvExerciseNumTv.background = ContextCompat.getDrawable(holder.itemView.context, R.drawable.ic_each_rv_ex_selected_white_bg)
                holder.tvExerciseNumTv.setTextColor(Color.WHITE)
            }
            else -> {
                holder.tvExerciseNumTv.background = ContextCompat.getDrawable(holder.itemView.context, R.drawable.ic_each_rv_ex_grey_bg)
